<template>
  <AdminLayout :locale="locale" :messages="messages">
    <Head :title="messages.adminPanel?.paymentsTitle || 'Оплаты (user_transactions)'" />

    <div class="space-y-6">
      <h1 class="text-2xl font-black text-dark">{{ messages.adminPanel?.paymentsTitle || 'Оплаты' }}</h1>
        ...
        <table class="w-full min-w-[720px] text-left text-sm">
          <thead class="border-b border-slate-200 bg-slate-50 text-xs font-bold uppercase text-slate-600">
            <tr>
              ...
              <AdminThSort :label="messages.adminPanel?.paymentsColUser || 'User ID'" column="user_id" :sort="filters.sort" :dir="filters.dir" :basePath="basePath" :common="filters" />
              <AdminThSort :label="messages.adminPanel?.paymentsColAmount || 'Сумма ₽'" column="amount_rub" :sort="filters.sort" :dir="filters.dir" :basePath="basePath" :common="filters" />
              <AdminThSort :label="messages.adminPanel?.paymentsColPlan || 'Мес.'" column="plan_months" :sort="filters.sort" :dir="filters.dir" :basePath="basePath" :common="filters" />
              <AdminThSort :label="messages.adminPanel?.paymentsColProvider || 'Канал'" column="provider" :sort="filters.sort" :dir="filters.dir" :basePath="basePath" :common="filters" />
              <th class="px-3 py-3">Ext. ID</th>
          ...
          <tbody>
            <tr v-for="r in rows.data" :key="r.id" class="border-b border-slate-100">
              ...
              <td class="max-w-[120px] truncate px-3 py-2 text-xs">{{ r.provider }}</td>
              <td class="max-w-[140px] truncate px-3 py-2 font-mono text-xs">
...
const basePath = computed(() => props.locale === 'en' ? '/en/admin/payments' : '/admin/payments');
